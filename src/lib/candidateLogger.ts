...
export async function logCandidateAction({ action, candidate, oldData, actor }: LogActionParams): Promise<void> {
  let changes: string | null = null;

  if (action === 'update' && oldData) {
    const diff: Record<string, { from: unknown; to: unknown }> = {};
    for (const f of ALL_FIELDS) {
      const oldVal = (oldData as Record<string, unknown>)[f] ?? '';
      const newVal = (candidate as Record<string, unknown>)[f] ?? '';
