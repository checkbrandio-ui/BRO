  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[rgba(123,63,191,0.08)] border border-[rgba(123,63,191,0.2)]">
        ...
        {f.form_token && (
          <a href={`/anketa-kandidata/${f.form_token}?edit=1`} target="_blank" rel="noreferrer"
      ...
      {/* Сгенерированные документы из карточки кандидата */}
      {candidate?.documents?.filter(d => d.type === 'generated').length > 0 && (
        <div className="bg-[#C9A84C]/5 border border-[#C9A84C]/20 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-xs font-bold text-[#C9A84C] uppercase tracking-widest">Сгенерированные документы ({candidate.documents.filter(d => d.type === 'generated').length})</h4>
          </div>
          <div className="grid sm:grid-cols-2 gap-2">
            {candidate.documents.filter(d => d.type === 'generated').map((doc, i) => (
              <div key={i} className="flex items-center gap-2 px-3 py-2 bg-[#C9A84C]/5 border border-[#C9A84C]/15 rounded-lg">
                <FileText size={13} className="text-[#C9A84C] flex-shrink-0" />
                <div className="min-w-0 flex-1">
                  <div className="text-xs text-[#F8FAFC]/70 truncate">{doc.name}</div>
                  {doc.uploaded_at && <div className="text-[10px] text-[#F8FAFC]/30">{new Date(doc.uploaded_at).toLocaleDateString('ru-RU')}</div>}
                </div>
                <a href={doc.url} target="_blank" rel="noreferrer"
                  className="p-1 rounded hover:bg-[#C9A84C]/15 text-[#F8FAFC]/50 hover:text-[#C9A84C] transition-all flex-shrink-0">
                  <Download size={12} />
                </a>
              </div>
            ))}
