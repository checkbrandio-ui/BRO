      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-3">
          {user ? (
            ...
            <Link to="/agency-login" className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[rgba(123,63,191,0.35)] text-[#7B3FBF] text-sm font-bold hover:bg-[#7B3FBF]/10 transition-all">
              <LogIn size={15} /> Войти
            </Link>
          )}
          <button
      ...
      {open && (
        <div className="lg:hidden bg-[#05070A]/98 backdrop-blur-xl border-t border-[rgba(123,63,191,0.15)] px-6 py-5 space-y-3">
          ...
          {user ? (
            ...
            <Link to="/agency-login" onClick={() => setOpen(false)} className="flex items-center gap-2 text-sm text-[#7B3FBF] font-bold py-2">
              <LogIn size={14} /> Войти для агентств
            </Link>
          )}
          <button
            onClick={() => handleAnchor('contacts')}
